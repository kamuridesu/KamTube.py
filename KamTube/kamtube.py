...
class KamTube:
    ...
    async def search(
        ...
        return json.loads(
            (await get(self.session, self.base_api_url + full_query)).decode("utf-8")
        )

    async def getMediaMetadata(self, media_id: str) -> Union[dict, list]:
        media_id = urlParse(media_id)
        result = (
            await get(self.session, self.base_api_url + f"videos/{media_id}")
        ).decode("utf-8")
        if "error" in result:
            try:
    ...
    async def getMediaInfo(self, media_id: str) -> dict[str, Union[str, list]]:
        media_id = urlParse(media_id)
        data = (await get(self.session, self.video_html_page + media_id)).decode(
            "utf-8"
        )
        return parse(data)
    ...
    async def processDownloadBody(
        self, media_id: str, media_type: str, quality: Union[None, int, float]
    ) -> dict[str, Any]:
        if media_type == "subs":
            raise NotImplemented("Subs are not implemented yet!")
        if media_type not in ["mixed", "video", "audio", "subs"]:
            raise TypeError(
                "Media type can only be 'mixed', 'video', 'audio' or 'subs'"
            )
    ...
    async def download(
        ...
        body_query: str = urllib.parse.quote(
            f"id={body['id']}&title={body['title']}&download_widget=",
            safe="~@#$&()*!+=:;,?/'",
        ) + urllib.parse.quote(body["query"], safe="~()*!'")
        self.logger.info(
            "Downloading media " + body["title"] + " with query " + body_query
        )
        try:
            _bytes = await post(
                self.session,
            ...
            self.logger.error(
                "Download failed! Reason may be something from " + str(e.__class__)
            )
            raise
    ...
    async def save(
        ...
        response: dict[str, Union[str, bytes]] = await self.download(
            media_id, media_type, quality
        )
